...
# === CORE EXTRACTION FOR TRANSLATION ONLY ===
def build_translation_only_corpus(verse_path, translation_path, book_code, book_id_map):
    ...
    for span_id, verse_num in verse_ids.items():
        if span_id in translations:
            ...
            # On suppose ici que le chapitre est connu par le nom du verset
            match_full = re.search(r"(\d+):(\d+)", verse_num)
            if match_full:
                chapter = match_full.group(1)
                verse = match_full.group(2)
            else:
                # fallback : on encode juste le numéro sans chapitre
                chapter = "1"
                verse = verse_number
...
if __name__ == "__main__":
    print("=== Extraction des traductions anglaises pour 1 Corinthians ===")
    ...
    print(f"\n✅ Fichier généré : 1corinthians_english_translation.csv ({len(df)} versets)")
